...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # bs, emb_dim, 128//16=8, 1024//16=64 -> bs, emb_dim, 512 -> bs, 512, emb_dim 
...
class CAVMAE(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        self.patch_embed_a.num_patches = int(audio_length * 128 / 256) # 512
    ...
    def forward_encoder(self, a, v, mask_ratio_a, mask_ratio_v, mask_mode='unstructured'):
        # embed patches
        a = a.unsqueeze(1) # bs,1,1024,128
        a = a.transpose(2, 3) # bs,1, 128, 1024
        ...
        if mask_mode == 'unstructured':
            a, mask_a, ids_restore_a = self.random_masking_unstructured(a, mask_ratio_a) # mask_ratio_a=0.75
        ...
        # print('a shape after blocks:', a.shape)
        # print('v shape after blocks:', v.shape)    
        ...
        # print('x shape after blocks:', x.shape) 
        x = self.norm(x)

        for blk in self.blocks_u:
            ca = blk(a, 'a')
        ca = self.norm_a(ca)
        # print('ca shape after blocks:', ca.shape) 

        for blk in self.blocks_u:
            cv = blk(v, 'v')
        cv = self.norm_v(cv)
        # print('cv shape after blocks:', cv.shape) 
    ...
    def forward_decoder(self, x, mask_a, ids_restore_a, mask_v, ids_restore_v):
        ...
        # 1,1,512
        # mask_a[0].sum() -> 128*768 ??
        # print(mask_a[0].sum()) 这个就是被mask掉的数目
        mask_tokens_a = self.mask_token.repeat(x.shape[0], int(mask_a[0].sum()), 1)
        # print('mask_tokens_a shape:',mask_tokens_a.shape) # bs, 384, 512
        # patch_embed_a.num_patches=512， int(mask_a[0].sum())=384
        # 减去后表示没有被mask的数据 
        # cat([bs, 128, 512] + [bs, 384, 512])
...
if __name__ == '__main__':
    model = CAVMAE()
    bs = 8
    a_input = torch.randn(bs, 1024, 128)
    v_input = torch.randn(bs, 3, 224, 224)
    loss, loss_mae, loss_mae_a, loss_mae_v, loss_c, mask_a, mask_v, c_acc =model(a_input, v_input)
